...
class Nav {
	 ...
	 * @var string
	 */
	protected $handle_prefix = 'airl-nav';
	...
	public function init() {
		if ( apply_filters( 'airl_disable_nav_mods', false ) ) {
			return;
		}
	...
	public static function nav_menu( $args, $toggle_label = false, $nav_classes = 'main-navigation nav-menu' ) {
		if ( false === $toggle_label ) {
			$toggle_label = '<span class="screen-reader-text">' . esc_html__( 'Expand Menu', 'airl' ) . '</span><div class="menu-toggle-icon"><span></span><span></span><span></span></div>';
		}
		?>
	...
	public function add_nav_sub_menu_buttons( $item_output, $item, $depth, $args ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$item_output .= '<button onClick="alxMediaMenu.toggleItem(this)"><span class="screen-reader-text">' . esc_html__( 'Toggle Child Menu', 'airl' ) . '</span><svg class="svg-icon" aria-hidden="true" role="img" focusable="false" xmlns="http://www.w3.org/2000/svg" width="14" height="8" viewBox="0 0 20 12"><polygon fill="" fill-rule="evenodd" points="1319.899 365.778 1327.678 358 1329.799 360.121 1319.899 370.021 1310 360.121 1312.121 358" transform="translate(-1310 -358)"></polygon></svg></button>';
		}
	...
	public function nav_menu_item_title( $title, $item, $args, $depth ) {
		...
		if ( $is_current ) {
			...
			return sprintf(
				...
				__( '<span class="screen-reader-text">%1$s </span>%2$s', 'airl' ),
				esc_html__( 'Current Page:', 'airl' ),
				$title
			);
		}
		...
		if ( $is_current_parent ) {
			...
			return sprintf(
				...
				__( '<span class="screen-reader-text">%1$s </span>%2$s', 'airl' ),
				esc_html__( 'Current Page Parent', 'airl' ),
				$title
			);
		}
