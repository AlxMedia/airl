<div class="page-title group">
	...
	<?php if ( is_home() ) : ?>
		<h2><?php echo airl_blog_title(); ?><i class="fas fa-pencil-alt"></i></h2>
	...
	<?php elseif ( is_search() ): ?>
		<h1>
			...
			<?php $search_results=$wp_query->found_posts;
				if ($search_results==1) {
					echo $search_results.' '.esc_html__('Search result','airl');
				} else {
					echo $search_results.' '.esc_html__('Search results','airl');
				}
			?>
		</h1>
		<div class="notebox">
			<?php esc_html_e('For the term','airl'); ?> "<span><?php echo get_search_query(); ?></span>".
			<?php if ( !have_posts() ): ?>
				<?php esc_html_e('Please try another search:','airl'); ?>
			<?php endif; ?>
	...
	<?php elseif ( is_404() ): ?>
		<h1><i class="fas fa-exclamation-circle"></i><?php esc_html_e('Error 404.','airl'); ?> <span><?php esc_html_e('Page not found!','airl'); ?></span></h1>
		<div class="notebox">	
			<p><?php esc_html_e( 'The page you are trying to reach does not exist, or has been moved. Please use the menus or the search box to find what you are looking for.', 'airl' ); ?></p>
			<?php get_search_form(); ?>
		</div>
